...
#if defined(__unix__) || defined(__linux__) || defined(unix) || \
    defined(linux) || defined(__unix) || defined(__linux)

char* ERROR_STRING = "Unknown error ";
#define S21_MAXERROR 134
#define S21_MINERROR -1
char* ERRORS[] = {
...
#define ERROR_STRING "Unknown error: "
#define S21_MAXERROR 107
#define S21_MINERROR -1
...
char* s21_strerror(int errnum) {
  ...
  char res_with_number[70] = {'\0'};
  char number[50] = {'\0'};
  char revers_number[50] = {'\0'};
  int is_negativ = 0;
  if (errnum < 0 || errnum >= S21_MAXERROR) {
    int len = 0;
    if (errnum < 0) {
      is_negativ = 1;
      errnum *= -1;
    ...
    while (errnum > 0) {
      revers_number[len] = (errnum % 10) + '0';
      len++;
      errnum = errnum / 10;
    }
    for (int i = 0; i < len; i++) {
      number[i] = revers_number[len - 1 - i];
    }
    number[len] = '\0';
    if (is_negativ) {
      s21_sprintf(res_with_number, "%s%c%s", ERROR_STRING, minus, number);
    } else {
      s21_sprintf(res_with_number, "%s%s", ERROR_STRING, number);
    }
    s21_strcat(res, res_with_number);
  } else {
    s21_strcat(res, (char*)ERRORS[errnum]);
  }
  return (char*)res;
}
